...
class GridMDP(MDP):
    ...
    def __init__(self, grid, terminals, init=(0, 0), gamma=.9):
        ...
        for x in range(self.cols):
            for y in range(self.rows):
                if grid[y][x]:
                    ...
                    reward[(x, y)] = grid[y][x]
        self.states = states
        actlist = EAST, NORTH, WEST, SOUTH = [(1, 0), (0, 1), (-1, 0), (0, -1)] #orientations
        transitions = {}
        for s in states:
            transitions[s] = {}
            for a in actlist:
        ...
        self._turns = LEFT, RIGHT = (+1, -1)
        self._orientations = EAST, NORTH, WEST, SOUTH = [(1, 0), (0, 1), (-1, 0), (0, -1)]
    ...
    def turn_heading(self,heading, inc, headings = self._orientations):
    ...
    def calculate_T(self, state, action):
        if action:
            return [(0.8, self.go(state, action)),
                    (0.1, self.go(state, self._turn_heading(action, RIGHT))),
                    (0.1, self.go(state, self._turn_heading(action, LEFT)))]
        else:
            return [(0.0, state)]
